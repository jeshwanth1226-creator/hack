...
def get_dashboard_view():
    user_list = []
    if 'active_users' in globals() and isinstance(active_users, list):
        user_list = active_users
    elif 'users' in globals() and isinstance(users, list):
        user_list = users
    elif 'users_db' in globals() and isinstance(users_db, list):
        user_list = users_db
    ...
    <body class="bg-slate-950 text-slate-100 min-h-screen p-8 font-sans">
        <div class="max-w-6xl mx-auto space-y-6">
            <div class="flex items-center justify-between border-b border-slate-800 pb-4">
                <div>
                    <h1 class="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400">
                        Green Wave Corridor | Live Dispatch Registry
                    </h1>
                    <p class="text-slate-400 text-sm mt-1">Real-time Emergency Vehicle & Traffic Signal Synchronization</p>
                </div>
                <div class="text-right">
                    <span class="inline-flex items-center gap-2 px-3 py-1 text-xs font-medium rounded-md bg-slate-900 border border-slate-800 text-slate-300">
                        <span class="w-2 h-2 rounded-full bg-emerald-400 animate-ping"></span>
                    ...
                    <p class="text-xs text-slate-500 mt-1">Active Units: <strong class="text-emerald-400">{len(user_list)}</strong></p>
                </div>
            </div>
            <div class="overflow-x-auto rounded-xl border border-slate-800 bg-slate-900/70 shadow-2xl backdrop-blur">
                <table class="w-full text-left text-sm">
                    <thead class="bg-slate-800/90 text-xs uppercase tracking-wider text-slate-400 border-b border-slate-700">
                        <tr>
                            ...
                            <th class="px-6 py-4">Vehicle ID</th>
                            <th class="px-6 py-4">Driver / Officer</th>
                            <th class="px-6 py-4">Target Hospital</th>
                            <th class="px-6 py-4">Logged In At</th>
                            <th class="px-6 py-4">Status</th>
                        </tr>
                    </thead>
